...
if __name__ == "__main__":
    ...
    for i in range(5):
        ...
        # gripper.move_and_waits_for_pos(0, 120, 120)  
        while JiTing == False:
            cnt +=1
...
if __name__ == "__main__":
    ...
    joint_init=np.array([-1.51,-0.38,-2.14,-2.14,1.63,-180/57.3])
    joint_init0 = np.array([-4.78,-2.14,2.01,-1.51,4.75,-130.62/57.3])
    joint_init = np.array([-4.78,-2.14,2.01,-2.54,4.78,-130.62/57.3])
    # joint_init = np.array([-273.96,-122.65,115.42,-145.6,273.95,-132.22])/57.3                                                                                          

   
    print('before',rtde_r.getActualQ())
    # rtde_c.moveJ(joint_init0,0.5,0.2)
    print('init')
    ...
    # try:
    #     T_target0  = np.array(ur5e_car.get_end_effector_posture(car_sub_record[-1][:3] + arm_state))
    #     rx0_in_arm = rtde_r.getActualTCPPose()
    #     print('T0',T_target0)
    #     print('r0',rx0_in_arm)
    # except:
    #     rtde_c.disconnect()
    #     print('err')
    #     sys.exit()
    # T_target0 = np.array([[ 0.0229687,   0.9971852 ,  0.07137309 , 0.48909649],
    #         [ 0.99912928 ,-0.02538341  ,0.03311143  ,0.07341334],
    #         [ 0.03482992  ,0.07055041 ,-0.99689995  ,1.07794465],
    #         [ 0.          ,0.          ,0.          ,1.        ]])init
    ...
    T_target[0,-1] = 1.5
    T_target[2,-1] = T_target0[2,-1] - 0.05
